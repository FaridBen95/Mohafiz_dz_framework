package com.MohafizDZ.framework_repository.datas;

import com.MohafizDZ.own_distributor.BuildConfig;

public class MConstants {
    public static final String DATABASE_NAME = "my_database.db";
    public static final String PROJECT_ID = "own-distributor";
    public static final String KEY_ACCOUNT_TYPE = "com.mohafizDZ.own_distributor.auth";
    public static final String DEFAULT_ANDROID_NAME = "MohafizDZ_" + BuildConfig.APPLICATION_ID +"_default";
    public static final String LOG_FILE_NAME = BuildConfig.APPLICATION_ID + "_log.txt";
    public static final String FIREBASE_STORAGE_LINK = "attachment";
    public static final String REPORT_TABLE_NAME = "report";
    public static final int LARGE_IMAGE_SIZE_LIMIT = 153600;
    public static String applicationFolder;
    public static String applicationImagesFolder;
    public static final String FCM_SERVER_KEY = "AAAAxCKNIx0:APA91bG8IN95DpU4RtTJKM5mnVi98HEA_AGNZBvDmRGg-iJM6NZNsd4Q9ixyeJDwRVk4WzrBr7_EeBXhPRN7zQicGToE4yQBHvPc_7xGaNEX3Jtwzu-9bzez-Tne6POULTKGpofmmiEQ";
    public static final String DEFAULT_CURRENCY = "DA";

    public static final int DATABASE_VERSION = 105;
    public static final int FRAMEWORK_VERSION = 9;
    //updated from cfeed on 09/07/2024
    //need to upgrade framework based on this project
}
